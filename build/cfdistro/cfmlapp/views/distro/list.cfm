...
<table border="0" cellspacing="0">
	...
	<tbody>
		...
		<cfloop collection="#local.distros#" item="local.id">
			...
			<tr>
<cfoutput>
				<td><a href="#resp.encodeURL('index.cfm?action=distro.view&id=#local.id#')#">#local.id#</a></td>
				<td><a href="#resp.encodeURL('index.cfm?action=distro.view&id=#local.id#')#">#local.distro.getDistroName()#</a></td>
				<td>#local.distro.getBuildDirectory()#</td>
				<!--- <td>#local.distro.getDepartment().getName()#</td> --->
				<td><a href="#resp.encodeURL('index.cfm?action=distro.delete&id=#local.id#')#">DELETE</a></td>
</cfoutput>
			</tr>
		</cfloop>
	</tbody>
